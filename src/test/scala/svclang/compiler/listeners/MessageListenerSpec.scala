...
class MessageListenerSpec extends SvcLangSpec {
  describe("compiling a message"){
    ...
    it("should extract fields"){
      val msg = Parse.messages("partial message A{ description string}").get.values.head
    ...
    describe("partial message"){
      it("should extract a partial message"){
        val msg = Parse.messages("partial message A").get.values.head
        msg shouldBe a [PartialMessage]
      }
    }
    describe("event"){
      it("should extract an event"){
        val evt = Parse.messages("event A").get.values
    ...
    describe("document"){
      it("should extract a document"){
        val doc = Parse.messages("document A").get.values
    ...
    describe("query"){
      it("should extract a query"){
        val qry = Parse.messages("query A responds with B").get.values
        qry.size shouldNot be (0)
        qry.head shouldBe a [Query]
      }
      it("should extract a ref to the responds with message"){
        val msgs = Parse.messages("query A responds with B").get
      ...
      it("should extract an inline response message and provide a ref to it"){
        val msgs = Parse.messages("query A responds with B extends hi.C").get
    ...
    describe("command"){
      it("should extract a command"){
        val cmd = Parse.messages("command A emits B fails with C").get.values
        cmd.size shouldNot be (0)
        cmd.head shouldBe a[Command]
      }
      it("should extract a ref to an emits message"){
        val msgs = Parse.messages("command A emits B").get
      ...
      it("should extract an inline emits event and provide a ref to it"){
        val msgs = Parse.messages("command A emits B extends C").get
        msgs.size should be (2)
        val cmd = msgs("A").asInstanceOf[Command]
      ...
      it("should extract a fails with message"){
        val msgs = Parse.messages("command A fails with B").get
        msgs.size should equal (1)
        val cmd = msgs("A").asInstanceOf[Command]
      ...
      it("should extract an inline fails with and provide a ref to it"){
        val msgs = Parse.messages("command A fails with B extends C").get
        msgs.size should be (2)
        val cmd = msgs("A").asInstanceOf[Command]
