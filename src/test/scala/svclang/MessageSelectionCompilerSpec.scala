...
class MessageSelectionCompilerSpec extends SvcLangSpec{
  it("should parse a message selection"){
    val svc = Compiler.compileService("service A input stream B { messages C }").get
  ...
  it("should compile a list of selections in a single declaration"){
    val svc = Compiler.compileService("service A input stream B { messages C,D*, E}").get
  ...
  it("should concat multiple messages declarations"){
    val svc = Compiler.compileService("service A input stream B { messages C,D* messages E*,F }").get
